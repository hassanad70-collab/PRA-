...
export function ImproveResumeDialog({ resumeId }: { resumeId: string }) {
  const t = useTranslations("Candidate.ImproveResume");
  ...
  const handleOpen = (next: boolean) => {
    ...
    if (next && !result) {
      startTransition(async () => {
        ...
        if (!res.success) toast.error(res.error ?? t("failed"));
        setResult(res);
      });
    }
  };
    ...
    <Dialog open={open} onOpenChange={handleOpen}>
      <DialogTrigger asChild>
        <Button variant="gradient">
          <Sparkles className="h-4 w-4" /> {t("trigger")}
        </Button>
      </DialogTrigger>
      <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
        ...
        {result?.success && result.improvement && (
          <div className="space-y-6 text-sm">
            <section>
              <h4 className="mb-2 font-semibold">{t("improvedSummary")}</h4>
              <p className="rounded-lg bg-secondary/60 p-3 text-muted-foreground">{result.improvement.improved_summary}</p>
            </section>

            <section>
              <h4 className="mb-2 font-semibold">{t("improvedExperience")}</h4>
            ...
            <div className="grid grid-cols-2 gap-4">
              <section>
                <h4 className="mb-2 font-semibold">{t("suggestedSkills")}</h4>
              ...
              <section>
                <h4 className="mb-2 font-semibold">{t("atsKeywords")}</h4>
                <div className="flex flex-wrap gap-1.5">
            ...
            {!!result.improvement.generated_achievement_statements.length && (
              <section>
                <h4 className="mb-2 font-semibold">{t("achievementIdeas")}</h4>
