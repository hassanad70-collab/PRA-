...
interface SubscriptionPlansProps {
  ...
  t: {
    ...
    upgradeCareerProLabel: string;
    upgradeModalTitle: string;
    upgradeModalDesc: string;
    upgradeModalClose: string;
  };
}
...
export function SubscriptionPlans({ currentPlan = "free", t }: SubscriptionPlansProps) {
  ...
  const plans: Plan[] = [
    ...
    },
    {
      ...
      highlight: true,
      premium: false,
      upgradeLabel: t.upgradeProfessionalLabel,
    },
    {
      id: "career_pro",
      ...
      highlight: false,
      premium: true,
      upgradeLabel: t.upgradeCareerProLabel,
    },
  ];
  ...
  return (
    <>
                  ...
                  <ul className="flex-1 space-y-2.5">
                    {plan.features.map((feature, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm">
                        ...
                        />
                        <span className={i === 0 && plan.id !== "free" ? "font-medium text-foreground" : "text-muted-foreground"}>
                          {feature}
                        </span>
                      </li>
                    ))}
                  ...
                  <div>
                    {isCurrent ? (
                      ...
                      <Button
                        ...
                        onClick={() => setUpgradeModalPlan(plan.name)}
                      >
                        {plan.upgradeLabel}
                      </Button>
                    )}
      ...
      {/* Upgrade modal */}
      <Dialog open={upgradeModalPlan !== null} onOpenChange={(open) => !open && setUpgradeModalPlan(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t.upgradeModalTitle}</DialogTitle>
            <DialogDescription>
              {t.upgradeModalDesc.replace("{plan}", upgradeModalPlan ?? "")}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button onClick={() => setUpgradeModalPlan(null)}>{t.upgradeModalClose}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
