...
type InterviewRow = Awaited<ReturnType<typeof getCandidateInterviews>>[number];
...
export default async function CandidateInterviewsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const user = await getCurrentUser();
  if (!user) redirect({ href: "/login", locale });

  const [interviews, t] = await Promise.all([getCandidateInterviews(user.id), getTranslations("Candidate.Interviews")]);
...
function InterviewCard({ interview, t }: { interview: InterviewRow; t: Translate }) {
  return (
    <Link href={`/candidate/jobs/${interview.application?.job?.id ?? ""}`}>
      <Card className="transition-shadow hover:shadow-md">
        <CardContent className="flex flex-wrap items-center justify-between gap-3 pt-6">
          <div className="min-w-0">
            <p className="truncate font-medium">{interview.application?.job?.title}</p>
            <p className="truncate text-sm text-muted-foreground">{interview.application?.job?.company?.name}</p>
            <p className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
              <Calendar className="h-3.5 w-3.5" /> {formatDate(interview.scheduled_at)} · {interview.duration_minutes} min ·{" "}
              {t(`type.${interview.interview_type as InterviewType}`)}
            </p>
